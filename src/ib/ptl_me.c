                               ...
                               ptl_handle_me_t *me_handle_p)
{
    ...
    err =
        le_append_check(TYPE_ME, ni, pt_index, (ptl_le_t *)me_init, ptl_list,
                        search_op, (ptl_handle_le_t *)me_handle_p);
    if (err)
        goto err2;
#else
                 ...
                 ptl_handle_me_t *me_handle_p)
{
    ...
    err =
        me_append_or_search(MYGBL_ ni_handle, pt_index, me_init, ptl_list, 0,
                            user_ptr, me_handle_p);
    return err;
}
 ...
 *
 */
int _PtlTriggeredMEAppend(PPEGBL ptl_handle_ni_t ni_handle,
                          ptl_pt_index_t pt_index, ptl_me_t *me_init,
                 ...
                 void *user_ptr)
{
    ...
    err =
        me_append_or_search(MYGBL_ ni_handle, pt_index, me_init, 0, search_op,
                            user_ptr, NULL);
    return err;
}
...
void do_trig_me_op(buf_t *buf, ct_t *ct)
{
    ...
    switch (buf->me_op) {
        case TRIG_ME_APPEND:
            _PtlMEAppend(MYGBL_ buf->ni_handle, buf->pt_index, buf->me_init,
                         buf->ptl_list, buf->user_ptr, buf->me_handle_p);
            break;
        case TRIG_ME_UNLINK:
